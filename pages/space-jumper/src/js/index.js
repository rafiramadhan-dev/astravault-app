...
const imageAssets = {
  characters: [
    ...
    { id: 'gambar_hati', src: 'image/gambar hati.png', color: '#FF1493', emoji: '💖' },
    { id: 'gambar_musuh', src: 'image/pesawat musuh.png', color: '#00FF7F', emoji: '👾' }
  ],
  obstacles: [
...
function updateLoadingProgress() {
  ...
  document.getElementById('loadingText').textContent = `Loading ${loadedCount}/${totalImages} assets...`;
  ...
  try {
    ...
    switch (type) {
      case 'jump':
        ...
        oscillator.frequency.exponentialRampToValueAtTime(784.00, ctx.currentTime + 0.1);
      ...
      case 'doubleJump':
        ...
        oscillator.frequency.exponentialRampToValueAtTime(1046.50, ctx.currentTime + 0.2);
        gainNode.gain.setValueAtTime(0.3, ctx.currentTime);
      ...
      case 'coin':
        ...
        oscillator.frequency.setValueAtTime(1046.50, ctx.currentTime);
        oscillator.frequency.exponentialRampToValueAtTime(1318.51, ctx.currentTime + 0.1);
      ...
      case 'gameOver':
        const frequencies = [440, 415.30, 392, 369.99];
      ...
      case 'levelUp':
        const levelFreqs = [523.25, 659.25, 783.99, 1046.50];
      ...
      case 'victory':
        const victorySeq = [523.25, 659.25, 783.99, 1046.50, 1318.51, 1567.98];
...
soundToggle.addEventListener('click', () => {
  ...
  icon.setAttribute('icon', soundEnabled ? 'mdi:volume-high' : 'mdi:volume-off');
...
const victoryChangeCharacterButton = document.getElementById("victoryChangeCharacterButton");
...
const player = {
  ...
  draw() {
    ...
    if (asset && asset.loaded && asset.img) {
      ctx.drawImage(asset.img, -this.width / 2, -this.height / 2, this.width, this.height);
    } else {
  ...
  jump() {
    if (this.grounded) {
      ...
      createParticles(this.x + this.width / 2, this.y + this.height, 10, selectedColor, 15);
      playSound('jump');
    } else if (isDoubleJumpAvailable) {
      this.dy = this.jumpPower * 0.8;
      isDoubleJumpAvailable = false;
      createParticles(this.x + this.width / 2, this.y + this.height, 15, "#FF1493", 20);
...
function showLevelTransition() {
  ...
  document.getElementById('nextLevelTime').textContent = levelConfig[nextLevel].time;
...
function showVictoryScreen() {
  ...
  document.getElementById('victoryHighScore').textContent = `BEST: ${highScore}`;
...
function drawObstacles() {
  for (let obs of obstacles) {
    ...
    if (obs.asset && obs.asset.loaded && obs.asset.img) {
      ctx.drawImage(obs.asset.img, -obs.width / 2, -obs.height / 2, obs.width, obs.height);
    } else {
...
function updateObstacles() {
  for (let i = 0; i < obstacles.length; i++) {
    ...
    if (player.x + player.width - 8 > obstacles[i].x &&
...
function drawCoins() {
  for (let coin of coins) {
    ...
    if (coinAsset && coinAsset.loaded && coinAsset.img) {
      ctx.drawImage(coinAsset.img, -coin.radius, -coin.radius, coin.radius * 2, coin.radius * 2);
    } else {
...
function gameLoop(timestamp) {
  ...
  if (obstaclesPassed > 0 && obstaclesPassed % 5 === 0) {
    gameSpeed = Math.min(gameSpeed + speedIncrement, levelConfig[currentLevel].speed + 3);
    obstaclesPassed++;
  }
