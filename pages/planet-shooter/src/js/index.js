...
const sounds = {
  ...
  victory: function() {
    if (!soundEnabled) return;
    [440, 550, 660, 880, 1046.50, 1318.51].forEach((freq, i) => {
      const oscillator = audioContext.createOscillator();
      const gainNode = audioContext.createGain();
      oscillator.connect(gainNode);
      gainNode.connect(audioContext.destination);
  ...
  cascade: function() {
    ...
    [300, 400, 500].forEach((freq, i) => {
      ...
      gain.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + i * 0.05 + 0.15);
...
// ============ PLANET TYPES (IMAGES) ============
const planetTypes = [
  { id: 0, name: 'UFO', color: '#FF6B6B', image: 'Assets/PlanetShooter/Planet1.png' },
  { id: 1, name: 'Rocket', color: '#4ECDC4', image: 'Assets/PlanetShooter/Planet2.png' },
  { id: 2, name: 'Fire', color: '#FFA500', image: 'Assets/PlanetShooter/Planet3.png' },
  { id: 3, name: 'UFO2', color: '#45B7D1', image: 'Assets/PlanetShooter/Planet4.png' },
  { id: 4, name: 'Heart', color: '#FF1493', image: 'Assets/PlanetShooter/Planet5.png' },
  { id: 5, name: 'Enemy', color: '#98D8C8', image: 'Assets/PlanetShooter/Planet6.png' }
];
...
function drawShooter() {
  ...
  const glowGradient = ctx.createRadialGradient(0, 0, PLANET_RADIUS * 0.5, 0, 0, PLANET_RADIUS * 1.5);
...
function checkGridCollision() {
  ...
  if (shooter.y - PLANET_RADIUS <= 20) {
    attachPlanet(0, Math.round((shooter.x - PLANET_RADIUS) / (PLANET_RADIUS * 2)));
    return true;
  }
  ...
  if (matches.length >= 3) {
    ...
    matches.forEach(({ row, col }) => {
      if (grid[row][col]) {
        ...
        createExplosion(grid[row][col].x, grid[row][col].y, grid[row][col].color);
        setTimeout(() => {
          grid[row][col] = null;
        }, 200);
      }
...
function checkFloating() {
  ...
  if (floatingCount > 0) {
    ...
    score += cascadePoints;
    showBonusText(canvas.width / 2, canvas.height / 2, `+${cascadePoints} CASCADE!`);
